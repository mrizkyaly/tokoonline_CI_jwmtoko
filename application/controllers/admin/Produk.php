...
class Produk extends CI_Controller {
	...
	{
		// Ambil data produk yang akan diambil
		$produk 	= $this->produk_model->detail($id_produk);
		// Ambil data kategori
		$kategori 	= $this->kategori_model->listing();
		// Validasi Input
		...
		if($valid->run()) {
			...
			// edit produk tampa ganti gambar
			$i = $this->input;
				...
				$data = array(	'id_produk'			=>$id_produk,
								...
								'harga'				=>$i->post('harga'),
								'stok'				=>$i->post('stok'),
								// Disimpan file gambar lembar tidak diganti
								// 'gambar'			=>$upload_gambar['upload_data']['file_name'],
								'berat'				=>$i->post('berat'),
								'ukuran'			=>$i->post('ukuran'),
								'status_produk'		=>$i->post('status_produk')
		...
		$data = array(	'title'		=>	'Tambah Produk'.$produk->nama_produk,
						'kategori'	=>	$kategori,
						'produk'	=>	$produk,
						'isi'		=>	'admin/produk/edit'
					);
